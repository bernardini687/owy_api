...
describe('when given optional query parameters', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })

  it('passes them over to the yelp service', async done => {
    openWeatherMock.currentWeather.mockImplementationOnce(() =>
      Promise.resolve(currentWeatherSuccessResponse.data)
    )
    ...
    const otherQueryParams = { term: 'pizza', limit: '4', sort_by: 'rating', foo: 'bar' }
    const response = await request.get('/cities').query({ ids: 120120, ...otherQueryParams })
    ...
    expect(yelpMock.businessesByCoordinates).toHaveBeenCalledTimes(1)
    expect(yelpMock.businessesByCoordinates).toHaveBeenCalledWith(coord.lat, coord.lon, otherQueryParams)
